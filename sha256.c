...
typedef struct hmac_sha256_ctx_t {
	uint8_t buf[SHA256_BLOCKLEN];
	// key block buffer, not needed after init
...
void hmac_sha256_init(HMAC_SHA256_CTX *hmac, const uint8_t *key, uint32_t keylen) {
	...
	if (keylen <= SHA256_BLOCKLEN) {
		...
		for (int i = 0; i < 64; i++) {
			printf("777sha.buf[%d] is %x\n",i,sha->buf[i]);
		}
	}
	for (int i = 0; i < 64; i++) {
		printf("111hmac->buf[%d] is %x\n",i,hmac->buf[i]);
	}
